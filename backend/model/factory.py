    ...
    def create_model(self, model_type: str):
        ...
        try:
            if model_type == 'chat':
                ...
                # 向量化调用同样要有超时：检索是咨询链路的前置步骤，
                # embedding 卡住会把整个咨询请求拖死。超时后由检索层降级为
                # 「无参考资料」，咨询仍按通用常识作答，不返回错误。
                emb_timeout = float(rag_conf.get('embedding_timeout') or 15)
                emb_retries = int(rag_conf.get('embedding_max_retries') if rag_conf.get('embedding_max_retries') is not None else 1)
                model = OpenAIEmbeddings(
                    model=model_name,
                    api_key=api_key,
                    base_url=base_url,
                    check_embedding_ctx_length=False,  # 百炼兼容端点不支持按 token 切分，关闭以免报错
                    request_timeout=emb_timeout,
