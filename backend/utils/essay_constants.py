...
# 统一评分维度（依据 data/广东省中考作文评分标准.doc 制定，所有体裁共用）
# 维度满分之和必须等于 TOTAL_SCORE (50分)：
# 内容(24%) + 结构(20%) + 立意(24%) + 语言(24%) + 书面(8%) = 100%
# 说明：自 2026-09 起，所有作文体裁统一以《广东省中考作文评分标准.doc》为默认评分依据，
#       不再使用各体裁独立的 TXT 评分标准文件，因此维度与满分配置不再按体裁拆分。
# 维度名称收敛为固定五维（对应需求 D1）：内容 / 结构 / 立意 / 语言 / 书面。
UNIFIED_DIMENSIONS: List[str] = ['内容', '结构', '立意', '语言', '书面']

UNIFIED_MAX_SCORES: Dict[str, int] = {
    '内容': 12,
    '结构': 10,
    '立意': 12,
    '语言': 12,
...
DIMENSION_MAP: Dict[str, List[str]] = {
    '议论文': UNIFIED_DIMENSIONS,
    '记叙文': UNIFIED_DIMENSIONS,
    '说明文': UNIFIED_DIMENSIONS,
}

# 各体裁的维度满分值映射（统一为同一套满分）
DIMENSION_MAX_SCORES: Dict[str, Dict[str, int]] = {
    '通用': dict(UNIFIED_MAX_SCORES),
    '议论文': dict(UNIFIED_MAX_SCORES),
    '记叙文': dict(UNIFIED_MAX_SCORES),
    '说明文': dict(UNIFIED_MAX_SCORES),
}
