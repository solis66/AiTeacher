...
**不要加端口号。** frontend 容器映射的是宿主机 80，浏览器对 HTTP 默认就走 80。
写成 `http://<公网IP>:80` 也等价，但**写 `:8501` 一定打不开** —— 那是 backend 容器
在 compose 内网里的端口，没有映射到宿主机，只有 nginx 能访问到它。

公网 IP 从哪来（服务器上执行）：

```bash
curl -s http://100.100.100.200/latest/meta-data/eipv4    # 阿里云元数据服务
curl -s ifconfig.me                                       # 取不到时的兜底
```

**打不开时，按这四行对号入座 —— 每行的修法完全不同：**

| 现象 | 原因 | 怎么修 |
|:---|:---|:---|
| 浏览器转圈后超时；但服务器上 `curl -sI http://127.0.0.1/` 返回 200 | **安全组没放行 80**（阿里云默认只开 22） | 控制台 → 实例 → 安全组 → 配置规则 → 入方向 → 手动添加 TCP `80/80`、授权对象 `0.0.0.0/0` |
| 提示「拒绝连接 / 无法访问此网站」，且 `docker ps` 里看不到容器 | 容器没起来 | `docker compose ps -a` 看退出码，`docker compose logs --tail 50` 看是不是还卡在构建 |
| 页面能打开，但注册/登录报错、或接口返回一堆 HTML | nginx 反代没生效，或 backend 在反复重启 | `docker compose logs --tail 30 backend`；再 `curl -s http://127.0.0.1/health`（走 nginx 打后端） |
| 看到登录页「AI智能批改工作台」 | 通了 | 接着按下面四条验收 |

**本机自测与外部访问要分开看**：`curl http://127.0.0.1/health` 走的是容器→nginx 的内网路径，
**它正常不代表外网能进**（安全组是外网那一段的闸门，容器层看不见）。
反过来，外网打不开时先在服务器上跑这条，能立刻把「应用问题」和「网络问题」分开。
